...
bag_files_directory ='/home/user/bagfilesxht/pick_and_place_rosbag/0413_rosbag/green'#/ff2'#'/home/user/bagfilesxht2/pick_and_place_rosbag/0408_rosbag/left'##'/home/user/bagfilesxht2/pick_and_place_rosbag/0407_rosbag'#'/home/user/CODINGrepos/Imitation_Learning/record_data/' #sys.argv[1]
#output_file = sys.argv[1] + "/dataset"
dataset_dir=bag_files_directory+'/pick_and_place_human2'#'/pick_and_place_human'#'/home/user/bagfilesxht2/pick_and_place_rosbag/0407_rosbag'
task_time = 120
image_w = 320
image_h = 240
...
def process_bag_file_realtime_infer(bag_files_directory, bag_file, bag_topics,policy,pre_process,post_process,get_image_xht):
    ...
    if temporal_agg:  # so all_time_actions is a 400*500*state_dim thing
        #Now, I am going to make it num_queries*num_queries!
        #all_time_actions = torch.zeros([max_timesteps, max_timesteps + num_queries, state_dim]).cuda()   #just once!#the author of this paper didn't make it 工程化
    ...
    with rosbag.Bag(bag_file_path, 'r') as bag:#for me, read twice will be OK
            ...
            if topic == "/camera/color/image_raw/compressed":
                ...
                if(bodyimagecount>=5):#
                    ...
                    with torch.inference_mode():  # 顾名思义，inference#is it a good practice to put this command here?
                        ...
                        #    print("all_time_actions.shape:", all_time_actions.shape)  # (500,600,14)# torch.Size([1228, 1328, 7]) correct!
                        #    # #all_time_actions = 0*all_time_actions#torch.zeros([max_timesteps, max_timesteps + num_queries, state_dim]).cuda()
                        ...
                        if config['policy_class'] == "ACT":#all obtained from config#先保留这个接口/选项
                            ...
                            if tt % query_frequency == 0:  # the action only feeds in every query_frequency steps?
                                ...
                                #print("all_actions.shape:", all_actions.shape)  # (1,100,14)#torch.Size([1, 100, 7]) also correct
                            if temporal_agg:  # all_actions should have 100 in one of its dimension
                                #print("num_queries",num_queries)#100 is correct!
                                all_time_actions[tt%num_queries]=all_actions#that line is overwritten!
                                ...
                                if(tt>=num_queries-1):
                                    ...
                                    columnindex=(torch.arange(tt,tt-100,-1))%num_queries
                                else:
                                    rowindex=torch.arange(tt+1)
                                    columnindex=torch.arange(tt,-1,-1)
                                ...
                                actions_populated = torch.all(actions_for_curr_step != 0, axis=1)  # it is a mask#operating on the dimension that is 14 or 7
        ...
        shift=2#4#8#1#initially 1
        qposgt=qposgt[:-shift]
        actiongt=actiongt[shift:]
        actioninfer=actioninfer[:-shift]
        visualize_differences_more(qposgt, actiongt, actioninfer,
                                   plot_path=os.path.join(dataset_dir,#'rgegiotj',
...
if __name__ == '__main__':
    ...
    #max_timesteps = int(max_timesteps * 1)  # may increase for real-world tasks
    # is max_timesteps really used?
    num_rollouts = num_episodes#6#50  # or 51 when using 04_07?
    # episode_returns = []
    # highest_rewards = []
    ...
    for bag_file in os.listdir(bag_files_directory):
        if bag_file.endswith(".bag"):
            ...
            process_bag_file_realtime_infer(bag_files_directory, bag_file, bag_topics,policy, pre_process, post_process,get_image_xht)

            #cProfile.run('process_bag_file_realtime_infer(bag_files_directory, bag_file, bag_topics, policy, pre_process,post_process, get_image_xht)')
            #buffer = []#Now I know that episode_data is a list of Data objects

#profiler.disable()
#profiler.dump_stats('./output1.prof')