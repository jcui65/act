...
bag_files_directory ='/home/user/bagfilesxht/pick_and_place_rosbag/0413_rosbag/green'#'/home/user/bagfilesxht2/pick_and_place_rosbag/0408_rosbag/left'#'/home/user/gitrepos/act/realdataxht'#'/home/user/bagfilesxht2/pick_and_place_rosbag/0408_rosbag/left'#
#output_file = sys.argv[1] + "/dataset"
shift=2#4#8#1#used to be 1
dataset_dir=bag_files_directory+f'/pick_and_place_green_human_{shift}'#f'/pick_and_place_left_human_{shift}'#'/pick_and_place_human'#'/home/user/bagfilesxht2/pick_and_place_rosbag/0407_rosbag'
    ...
    with rosbag.Bag(bag_file_path, 'r') as bag:
        for topic, msg, t in bag.read_messages(topics=bag_topics):
            if topic == "/control/motor_angle":#it is a single angle!
                ...
                #print()
                anglef=angle#max(0,min(1,angle))#through observation, this step/filtering is not needed
                #print("angle before:",angle,"angle after:", anglef)
                #going to normalize it!
                motor_time_buffer.append(time)#it is a list
                motor_angle_buffer.append(anglef)
            '''
            if topic == "/tf_static":
        ...
        motor_interpnearest = interp1d(motor_time_buffer, motor_angle_buffer,kind='nearest', fill_value='extrapolate')
        joint_angle_interp = [
        ...
        for i in range(len(image_body_buffer)):
            ...
            '''
            if(i<len(image_body_buffer)-1):
                buffer_datap1 = image_body_buffer[i+1]#p1 means plus 1
                camera_timep1 = buffer_datap1.time#That is the only thing I need from t+1, right?
            else:
                #buffer_datap1=buffer_data
                camera_timep1=camera_time
            '''
            '''
            # deal with tf
            ...
            if joint_angle_interp is not None:
                ...
                #if(i<len(image_body_buffer) - 1):
                #    joint_anglep1 = interpolate_joint_angles(camera_timep1)#it is a list
                #else:
                #    joint_anglep1=joint_angle
                #buffer_datap1.joint_angle =joint_anglep1
                #buffer_data.action=joint_anglep1
                if len(episode_buffer) >= shift and episode_buffer[len(episode_buffer) - shift].joint_angle is not None:
                    #episode_buffer[len(episode_buffer) - 1].joint_angle = buffer_data.joint_angle.copy()  # .tolist()#now it has dimension 6
                    #episode_buffer[len(episode_buffer) - 1].action = buffer_datap1.joint_angle.copy()  # .tolist()#now it has dimension 6
                    #episode_buffer[len(episode_buffer) - 1].action = buffer_data.action.copy()  # .tolist()#now it has dimension 6
                    episode_buffer[len(episode_buffer) - shift].action = buffer_data.joint_angle.copy()  # .tolist()#now it has dimension 6

            if motor_interp is not None:
                motor_angleb = motor_interp(camera_time)  # get the angle at the camera time!
                #if only use the above,
                #here I should set/filter it to be between 0 and 1 inclusive!
                # print("motor_angle",motor_angle)#1
                if(motor_angleb>1 or motor_angleb<0):
                    motor_angle=motor_interpnearest(camera_time)#nearest will ensure that there is no value outside the region [0,1]
                else:
                    motor_angle = motor_angleb#max(min(motor_angleb,1),0)
                #print("motor_angle before:", motor_angleb, "motor_angle after:", motor_angle)

                buffer_data.motor_angle = motor_angle  # because up to now, the action is a list already!

                #if (i < len(image_body_buffer) - 1):
                #    motor_anglep1 = motor_interp(camera_timep1)
                #else:
                #    motor_anglep1 = motor_angle
                #buffer_datap1.motor_angle = motor_anglep1  # because up to now, the action is a list already!
                #buffer_data.action.append(motor_anglep1)
                ...
                if len(episode_buffer) >= shift and episode_buffer[len(episode_buffer) - shift].joint_angle is not None:
                    ...
                    #episode_buffer[len(episode_buffer) - 1].joint_angle.append(buffer_data.motor_angle.copy().tolist())  # now it has dimension 7
                    #episode_buffer[len(episode_buffer) - 1].action.append(buffer_datap1.motor_angle.copy().tolist())  # now it has dimension 7
                    episode_buffer[len(episode_buffer) - shift].action.append(motor_angle)  # now it has dimension 7
            ...
            if vel is not None:
                buffer_data.vel = vel
                    ...
                    #episode_buffer[len(episode_buffer) - 1].action.append(buffer_data.vel.copy())#need to be cautious before knowing what action stands for
            ...
            '''
            if (i==len(image_body_buffer)):
                if joint_angle_interp is not None:
                    buffer_data.action=buffer_data.joint_angle.copy()
                if motor_interp is not None:
                    buffer_data.action.append(buffer_data.motor_angle.copy())
            '''
...
if __name__ == '__main__':
    ...
    for bag_file in os.listdir(bag_files_directory):
        if bag_file.endswith(".bag"):
            ...
            for episode in episode_data[5:-max(5,shift)]:#[5:-5]:  # convert format skip first and last 5 stamps
